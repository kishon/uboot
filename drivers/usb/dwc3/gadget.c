 ...
 *
 * Taken from Linux Kernel v3.16 (drivers/usb/dwc3/gadget.c) and ported
 * to uboot.
 *
 * commit 5cd8c4 : usb: dwc3: gadget: check link trb after free_slot is
 *		   increased
 *
...
static int dwc3_alloc_trb_pool(struct dwc3_ep *dep)
{
	...
	dep->trb_pool = dwc3_dma_alloc_coherent(dwc->dev,
						sizeof(struct dwc3_trb) *
						DWC3_TRB_NUM, &dep->trb_pool_dma
						, GFP_KERNEL);
	if (!dep->trb_pool) {
...
static int dwc3_gadget_set_ep_config(struct dwc3 *dwc, struct dwc3_ep *dep,
				     ...
				     *comp_desc, bool ignore, bool restore)
{
	struct dwc3_gadget_ep_cmd_params params;
				 ...
				 unsigned node)
{
	...
	dev_vdbg(NULL, "%s: req %p dma %08llx length %d%s%s\n",
				...
				gfp_t gfp_flags)
{
	...
	if (!dep->endpoint.desc) {
		dev_dbg(NULL, "trying to queue request %p to disabled %s\n",
			request, ep->name);
		return -ESHUTDOWN;
	}

	dev_vdbg(NULL, "queing request %p to %s length %d\n",
...
static int dwc3_gadget_ep_set_halt(struct usb_ep *ep, int value)
{
	...
	if (usb_endpoint_xfer_isoc(dep->endpoint.desc)) {
		dev_err(NULL, "%s is of Isochronous type\n", dep->name);
		ret = -EINVAL;
		goto out;
	}
				  ...
				  const struct dwc3_event_devt *event)
{
	...
	case DWC3_DEVICE_EVENT_HIBER_REQ:
		if (!dwc->has_hibernation) {
			dev_WARN(dwc->dev, "unexpected hibernation event\n");
			break;
		}
...
int dwc3_gadget_init(struct dwc3 *dwc)
{
	...
	dwc->ctrl_req = dwc3_dma_alloc_coherent(dwc->dev, sizeof(*dwc->ctrl_req)
						, &dwc->ctrl_req_addr
						, GFP_KERNEL);
	if (!dwc->ctrl_req) {
	...
	dwc->ep0_trb = dwc3_dma_alloc_coherent(dwc->dev, sizeof(*dwc->ep0_trb),
	...
	dwc->ep0_bounce = dwc3_dma_alloc_coherent(dwc->dev, DWC3_EP0_BOUNCE_SIZE
						  , &dwc->ep0_bounce_addr
						  , GFP_KERNEL);
	if (!dwc->ep0_bounce) {
